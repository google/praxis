...
class RetroTransformer(Transformer):
  """Transformer layer for RETRO.

  This layer is adapted from the standard Transformer layer by enabling the
  chunked-crossattention layer.
  """

  # TODO(yuancao): Handle packed inputs properly (and make sure segment_pos is
  # correct).

  class HParams(Transformer.HParams):
    """Additional params for RETRO."""
  ...
  def setup(self) -> None:
    super().setup()
    p = self.hparams
    assert p.cca_tpl is not None, (
        'Chunked cross attention must be set for RETRO.')
    params = p.cca_tpl.clone()
    params.name = 'cca_layer'
    params.atten.input_dim = p.input_dims
    params.atten.hidden_dim = p.input_dims
    params.atten.num_heads = p.num_heads
    params.atten.dim_per_head = p.dim_per_head
    params.atten.atten_dropout_prob = p.atten_dropout_prob
  ...
  def __call__(self,
    ...
    Args:
      ...
      segment_pos: A JTensor of shape [B, T]. The position of each token in a
        segment.
      neighbors: Sequence of neighbor encodings, JTensor of shape [B, L, K, R,
        D], where L=num of chunks, K=num of neighbors (top-k), R=length of
        chunks.
...
class StackedRetroTransformer(StackedTransformer):
  ...
  def __call__(self,
    ...
    attention_mask, _ = compute_attention_masks_for_fprop(
        inputs,
        paddings,
        p.mask_self_attention,
        segment_mask,
...
class StackedRetroTransformerRepeated(StackedTransformerRepeated):
  """Repeated RetroTransformer blocks."""
